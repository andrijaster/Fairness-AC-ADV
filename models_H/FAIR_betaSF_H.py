...
class FAIR_betaSF_H_class():
    ...
    def __init__(self, input_size, num_layers_w,
                     step_w, num_layers_A, step_A,
                      num_layers_y, step_y):
        self.model = FAIR_betaSF_H_class.Fair_classifier(input_size, num_layers_w,
                     step_w, num_layers_A, step_A,
                      num_layers_y, step_y)
    ...
    def fit(self, x_train, y_train, A_train, max_epoch = 300, mini_batch_size = 50, 
        ...
        def loss_w(output_A, output_Y, target_Y, alpha, beta, dist, sam):
            output_A = torch.clamp(output_A, 1e-5, 1 - 1e-5)
            output_Y = torch.clamp(output_Y, 1e-5, 1 - 1e-5)
            ML = dist.log_prob(sam)*(loss(output_Y, target_Y, sam) - alpha*entropy(output_A) - beta * torch.mean(sam))
        ...
        for e in range(max_epoch):
            for i in range(0,x_train.size()[0], mini_batch_size):     
                ...
                loss1 = loss_w(A, y, batch_y, alpha, beta, dist, w)
                loss1.backward()
                optimizer_1.step()
