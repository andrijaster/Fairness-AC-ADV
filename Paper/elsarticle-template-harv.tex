...
\title{Structured Prediction of Sparse Dependent Variables for Traffic State Etimation in Large-Scale Networks \tnoteref{label1}}
\tnotetext[label1]{}
...
Experimental results on real-world applications are shown in section~\ref{Sec:exp-evaluation}. Final conclusions are given in section~\ref{Sec:Conclusion}.
...
Another adversarial approach focuses on learning to select non-sensitive features on per instance basis \cite{wang2019approaching}. The adversarial approach is employed to to minimize the correlation between selected features and sensitive information. 
...
For an instance $\mathbf{x}$ the weighting network outputs the weight of that instance $w_\mathbf{x}\in[0,1]$, while predictor network and sensitive network output prediction of output labels $\mathbf{y}$ and sensitive features $\mathbf{s}$, respectively. In order to incorporate fairness objective, FAIR weights log likelihood of instances, so that the ones which are strongly informative of sensitive attribute, but not of target variable are assigned low weights and the ones which are informative of target variable, but not of sensitive attributes are assigned high weights.
...
\subsection{Experimental behaviour of FAIR models}
