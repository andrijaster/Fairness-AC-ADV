...
\begin{theorem}
	...
	\label{pp:anypoint}
\end{theorem}
Moreover, the proof shows that an optimal weight is $1$ if it holds $\alpha\log P_{\psi^*}(\mathbf{s}|\mathbf{x})<\log P_{\phi^*}(\mathbf{y}|\mathbf{x})$ and that it is $0$ if it holds $\alpha\log P_{\psi^*}(\mathbf{s}|\mathbf{x})>\log P_{\phi^*}(\mathbf{y}|\mathbf{x})$. This property will be relevant for the intuitive discussion of the results.

The following theorem establishes the behaviour of the optimal weights at one extreme of the range of the hypermarameter $\alpha$.
\begin{theorem}
	If $\lambda$ is finite, for $\alpha=0$ it holds $\mathbf{w}^*=\mathbf{0}$.
	\label{pp:zero}
\end{theorem}

The following theorem establishes the behaviour of the optimal weights at another extreme of the range of the hypermarameter $\alpha$. We explicitly denote dependence of the optimal solution on $\alpha$.
...
$\alpha\log P_{\psi^*_\alpha}(\mathbf{s}|\mathbf{x})-\log P_{\phi^*_\alpha}(\mathbf{y}|\mathbf{x})\leq 0$. For other instances it holds $w^*_{\mathbf{x},\alpha}=0$ (see Theorem \ref{pp:anypoint} and the comment bellow its statement). Therefore the optimal solutions $\phi^*$ and $\psi^*$
...
\begin{theorem}
If $\lambda$ is finite, the $ASD_\alpha=|P_\alpha(\hat{\mathbf{y}}=1|\mathbf{s}=0)-P_\alpha(\hat{\mathbf{y}}=1|\mathbf{s}=1)|$ decreases to $0$ as $\alpha$ decreases.
\end{theorem}
The motivation for our work is the observation that the data is often not fair and that the weighting should be used in order to learn from fair instances. What the theorem intuitively states is that the $ASD$ of the distribution from which the optimal classifier is learned can actually be controlled by the hyperparameter $\alpha$, that way mitigating the bias in the data. 
...
\section{Proofs of Theoretic Results}
\label{app:proofs}
...
It holds
\begin{flalign*}
ASD_\alpha&=|P_\alpha(\hat{\mathbf{y}}=1|\mathbf{s}=0)-P_\alpha(\hat{\mathbf{y}}=1|\mathbf{s}=1)|\\
		  &=\left|\int P_{\psi^*_\alpha}(\mathbf{y}=1|\mathbf{x})P_\alpha(\mathbf{x}|\mathbf{s}=0)d\mathbf{x}-\int P_{\psi^*_\alpha}(\mathbf{y}=1|\mathbf{x})P_\alpha(\mathbf{x}|\mathbf{s}=1)d\mathbf{x}\right|\\
		  &\leq\int|P_{\psi^*_\alpha}(\mathbf{y}=1|\mathbf{x})P_\alpha(\mathbf{x}|\mathbf{s}=0)-P_{\psi^*_\alpha}(\mathbf{y}=1|\mathbf{x})P_\alpha(\mathbf{x}|\mathbf{s}=1)|d\mathbf{x}\\
		  &=\int P_{\psi^*_\alpha}(\mathbf{y}=1|\mathbf{x})|P_\alpha(\mathbf{x}|\mathbf{s}=0)-P_\alpha(\mathbf{x}|\mathbf{s}=1)|d\mathbf{x}\\
		  &=\int P_{\psi^*_\alpha}(\mathbf{y}=1|\mathbf{x})\left|\frac{P_\alpha(\mathbf{s}=0|\mathbf{x})P_\alpha(\mathbf{x})}{P_\alpha(\mathbf{s}=0)}-\frac{P_\alpha(\mathbf{s}=1|\mathbf{x})P_\alpha(\mathbf{x})}{P_\alpha(\mathbf{s}=1)}\right|d\mathbf{x}\\
		  &=\int P_{\psi^*_\alpha}(\mathbf{y}=1|\mathbf{x})\left|\frac{P_\alpha(\mathbf{s}=0|\mathbf{x})}{P_\alpha(\mathbf{s}=0)}-\frac{P_\alpha(\mathbf{s}=1|\mathbf{x})}{P_\alpha(\mathbf{s}=1)}\right|P_\alpha(\mathbf{x})d\mathbf{x}\\
\end{flalign*}
...
\end{proof}
Note that the value $\alpha_0$ would be arbitrarily close to $0$ for sufficiently large regularization hyperparameter value $\lambda$.
